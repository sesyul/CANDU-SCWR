    ...
    while Out < 1 : 
        while ConvD == False :
            ...
            if ConvD == True :
                ...
                if os.path.isfile(PATH_PROC_DJ+'MICROLIB.inp') == True :
                    os.remove(PATH_PROC_DJ+'MICROLIB.inp')
                    RoadMap = RoadMap + str(Step)+' old MICROLIB removed \n'
                else :
                    RoadMap = RoadMap + str(Step)+' old MICROLIB not found, error triggered \n'
                    END = time.time()
                    RoadMap = RoadMap +'\nTime spent during execution '+str(END-START)[0:8]+' seconds \n' 
                    with open(PATH_EXEC+'Exec_report.txt', 'w') as File:
                        File.write(RoadMap)
                ...
                if os.path.isfile(PATH_EXEC+'MICROLIB.out') == True :
                    shutil.copyfile(PATH_EXEC+'MICROLIB.out',PATH_PROC_DJ+'MICROLIB.inp')
                    RoadMap = RoadMap + str(Step)+' new MICROLIB copied in store \n'
                else :
                    RoadMap = RoadMap + str(Step)+' new MICROLIB not found, error triggered \n'
                    END = time.time()
                    RoadMap = RoadMap +'\nTime spent during execution '+str(END-START)[0:8]+' seconds \n' 
                    with open(PATH_EXEC+'Exec_report.txt', 'w') as File:
                        File.write(RoadMap)
            ...
            os.remove(PATH_EXEC+'MICROLIB.out')
            RoadMap = RoadMap + str(Step)+' new MICROLIB deleted from execution dir \n'
